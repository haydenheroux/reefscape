...
discretize(std::pair<SystemMatrix, InputMatrix> continuous_matrices,
  ...
  M.template block<2, 1>(0, 2) = continuous_matrices.second;
  M.template block<1, 3>(2, 0).setZero();

  Eigen::Matrix<double, 3, 3> phi = (M * sample_period.in(seconds)).exp();
  Eigen::Matrix<double, 2, 2> Ad = phi.template block<2, 2>(0, 0);
  Eigen::Matrix<double, 2, 1> Bd = phi.template block<2, 1>(0, 2);
